...
├── build/                    # generated by `daemon` (gitignored)
└── readme.md
```
...
7. **Optional — browse as a site** — run `daemon` (default
   http://localhost:8000, browsers auto-reload): it renders `build/` on start
   (wiped first, so edits and deletions are reflected) and rebuilds on any
   change under `problems/`, `notes/` or `frontend/`. There is no standalone
   build verb. The site is a single Vue 3 + BootstrapVueNext SPA
   (`frontend/`) over hash URLs: `#/problems/luogu/P4001` for a note,
   `#/notes/algos/trick` for a freeform note. There are no per-note HTML pages
   — every note's markdown is rendered server-side (katex math, TOC, heading
   permalinks) into `build/notes.json`, which the SPA fetches and renders with
   v-html; the sidebar tree comes from `build/tree.json`.
...
| `daemon` | build the site in `build/`, watch + rebuild on change + live-reload server (problems/, notes/, frontend/) |
