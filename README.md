...
├── build/                   # generated by `daemon` (gitignored) — SPA shell + notes.json + tree.json
└── readme.md
```
...
- **Site** — `daemon` renders every note (markdown-it + server-side katex, TOC,
  heading permalinks) into `build/notes.json` keyed by route path — there are no
  per-note HTML pages, and the initial build happens on daemon start (there is
  no standalone `build` verb). The site itself is a Vue 3 SPA (`frontend/`,
  vue-router hash URLs like `#/problems/luogu/P4001`, BootstrapVueNext, pinia)
  that fetches `notes.json` + `tree.json` and renders the note bodies with
  `v-html`; the daemon rebuilds on any change under `problems/`, `notes/` or
  `frontend/` and live-reloads browsers over SSE.
