...
pnpx tsx /path/to/myLearn/index.ts build                              # site into build/ (markdown-it + katex → notes.json; Vue SPA shell from frontend/)
```
...
├── build/                   # generated by `build` (gitignored) — SPA shell + notes.json + tree.json
└── readme.md
```
...
- **Site** — `build` renders every note (markdown-it + server-side katex, TOC,
  heading permalinks) into `build/notes.json` keyed by route path — there are no
  per-note HTML pages. The site itself is a Vue 3 SPA (`frontend/`, vue-router
  hash URLs like `#/problems/luogu/P4001`, BootstrapVueNext, pinia) that fetches
  `notes.json` + `tree.json` and renders the note bodies with `v-html`; the
  `daemon` rebuilds it and live-reloads browsers over SSE.
